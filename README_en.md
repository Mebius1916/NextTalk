...
Due to the use of NextJS's SSR and API route features, this program can only run in a development environment.

Regarding the production version: I separated the web part and packaged it into a desktop version using Pake. The production experience installation package is available in the release section, currently only for Windows.
